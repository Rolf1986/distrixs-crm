      ...
      <div className="bg-white border border-slate-200 rounded-lg p-6">
        <div className="flex items-start gap-4">
          ...
          <div className="flex-1 min-w-0">
            <p className="font-medium text-slate-900">{quote.quoteNumber}.pdf</p>
            <p className="text-sm text-slate-500 mt-0.5">
              {quote.customer.companyName} · {formatDate(quote.quoteDate)} · {formatCurrency(Number(quote.total))}
            </p>
          </div>
          <a
          ...
          <table className="w-full text-sm">
            ...
            <tfoot>
              <tr className="border-t border-slate-200">
                ...
                <td className="px-4 py-2 text-right text-slate-700 font-medium">{formatCurrency(Number(quote.subtotal))}</td>
              </tr>
              <tr>
                <td colSpan={3} className="px-4 py-1.5 text-right text-slate-500 text-xs">BTW 21%</td>
                <td className="px-4 py-1.5 text-right text-slate-500">{formatCurrency(Number(quote.vatAmount))}</td>
              </tr>
              <tr className="bg-brand-blue-light">
                ...
                <td className="px-4 py-2.5 text-right font-bold text-brand-blue">{formatCurrency(Number(quote.total))}</td>
              </tr>
            </tfoot>
          </table>
        </div>
