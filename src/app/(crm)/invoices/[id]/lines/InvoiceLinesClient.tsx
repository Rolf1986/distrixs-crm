...
export function InvoiceLinesClient({
  invoiceId,
  initialLines,
  products,
  paidAmount,
  ...
  async function deleteLine(lineId: string) {
    setDeleting(lineId);
    try {
      await fetch(`/api/invoices/${invoiceId}/lines/${lineId}`, { method: "DELETE" });
  ...
  const openAmount = total - paidAmount;

  // colspan: SKU, Omschrijving, Aantal, Bruto prijs, Korting %, BTW %, BTW bedrag, Regel totaal, Actions = 9
  const COL_COUNT = 9;
          ...
          <table className="w-full text-sm">
            ...
            <tfoot className="border-t-2 border-slate-200 bg-slate-50">
              ...
              {paidAmount > 0 && (
                <>
                  <tr>
                    <td colSpan={COL_COUNT - 2} className="px-4 py-2 text-right text-sm text-green-600">Betaald</td>
                    <td className="px-4 py-2 text-right text-green-700 font-medium">− {formatCurrency(paidAmount)}</td>
                    <td />
                  </tr>
                  <tr className="border-t border-slate-200">
                    <td colSpan={COL_COUNT - 2} className="px-4 py-3 text-right font-semibold text-slate-700">Openstaand</td>
                    <td className={`px-4 py-3 text-right font-bold text-base ${openAmount <= 0 ? "text-green-700" : "text-red-700"}`}>
        ...
        <div className="space-y-3">
          ...
          <div className="flex gap-3 items-end flex-wrap">
            ...
            <div className="ml-auto flex items-end">
              {addPrice && addQty && (
                <span className="text-sm text-slate-500 mr-3 pb-2">
                  = {formatCurrency(calcNet(Number(addPrice), Number(addQty), Number(addDiscount)))}
                </span>
              )}
              <button
                onClick={addLine}
