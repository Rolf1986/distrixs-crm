...
// Geen externe fonts — Helvetica is ingebouwd in @react-pdf/renderer
const S = StyleSheet.create({
  ...
  tableFooter: { flexDirection: "row", paddingVertical: 6, paddingHorizontal: 8, backgroundColor: "#f8fafc", marginTop: 1 },
...
export function QuotePdf({ data }: { data: QuotePdfData }) {
  ...
  return (
    <Document title={data.quoteNumber} author={coName}>
      <Page size="A4" style={S.page}>
        ...
        <View style={S.header}>
          ...
          <View style={{ alignItems: "flex-end" }}>
            ...
            <Text style={S.docMeta}>
              {`Nummer: ${data.quoteNumber}\nDatum: ${fmtDate(data.quoteDate)}\nGeldig tot: ${fmtDate(data.validUntil)}`}
            </Text>
          </View>
        </View>
        ...
        {(co?.quoteTerms || true) && (
          <View style={S.terms}>
            <Text>
              {co?.quoteTerms ?? "Alle bedragen zijn exclusief BTW tenzij anders aangegeven. Deze offerte is geldig tot de bovengenoemde datum."}
            </Text>
          </View>
        )}
        ...
        <View style={S.footer} fixed>
          <Text style={S.footerText}>
            {[coName, co?.email, co?.phone].filter(Boolean).join(" · ")}
          </Text>
