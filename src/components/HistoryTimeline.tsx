...
import { CreditCard, Trash2, RefreshCw, Info, Mail, Bell, Eye, X, Loader2, StickyNote, Plus } from "lucide-react";
...
export type HistoryItem =
  ...
  | {
      kind: "email";
      id: string;
      emailKind: "INVOICE" | "REMINDER" | "QUOTE";
      at: string;
      to: string;
      subject: string;
    }
...
function formatValue(action: string, value: string | null): string | null {
  ...
  try {
    ...
    if ((action === "invoice.status_changed" || action === "quote.status_changed") && typeof parsed === "string") {
...
const AUDIT_META: Record<string, { label: string; Icon: React.ElementType; color: string }> = {
  ...
  "quote.status_changed": { label: "Status gewijzigd", Icon: RefreshCw, color: "text-blue-500 bg-blue-50" },
...
/**
 * Herbruikbare geschiedenis-tijdlijn voor facturen én offertes.
 * basePath bv. "/api/invoices/<id>" of "/api/quotes/<id>".
 */
export function HistoryTimeline({ basePath, items }: { basePath: string; items: HistoryItem[] }) {
  ...
  async function viewEmail(emailId: string) {
    ...
    try {
      const res = await fetch(`${basePath}/emails/${emailId}`);
  ...
  async function addNote() {
    ...
    setSavingNote(true);
    try {
      const res = await fetch(`${basePath}/notes`, {
        method: "POST",
  ...
  return (
    <div className="max-w-2xl space-y-5">
      {/* Notitie toevoegen */}
      <div className="bg-white border border-slate-200 rounded-lg p-3 flex items-start gap-2">
        <StickyNote className="w-4 h-4 text-amber-500 mt-2 shrink-0" />
        <textarea
          value={note}
          onChange={(e) => setNote(e.target.value)}
          placeholder="Notitie toevoegen (bijv. afspraak gemaakt met klant op 20-07)…"
          rows={2}
          className="flex-1 resize-none border border-slate-200 rounded-lg px-3 py-2 text-sm focus:outline-none focus:ring-2 focus:ring-brand-blue/30"
        />
        <button
          onClick={addNote}
          disabled={savingNote || !note.trim()}
          className="flex items-center gap-1.5 bg-brand-blue text-white text-sm font-medium px-3 py-2 rounded-lg disabled:opacity-50 shrink-0"
        >
        ...
        <ol className="relative border-l border-slate-200 ml-4 space-y-0">
          {items.map((item, idx) => {
            ...
            if (item.kind === "email") {
              ...
              label = item.emailKind === "REMINDER" ? "Herinnering verstuurd" : item.emailKind === "QUOTE" ? "Offerte verstuurd" : "Factuur verstuurd";
              node = (
                <>
            ...
            } else if (item.action === "note.added") {
              Icon = StickyNote;
              color = "text-amber-600 bg-amber-50";
              label = "Notitie";
              node = (
                <>
                  <p className="text-sm text-slate-700 mt-1 whitespace-pre-wrap">{formatValue(item.action, item.newValue)}</p>
                  {item.user && <p className="text-xs text-slate-400 mt-1">door {item.user}</p>}
                </>
              );
            } else {
